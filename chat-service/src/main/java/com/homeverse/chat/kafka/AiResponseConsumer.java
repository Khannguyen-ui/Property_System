...
public class AiResponseConsumer {
    ...
    public void listenAiResponse(String message) {
        try {
            ...
            // Bắn thẳng kết quả về WebSocket của user đó theo kênh /topic/user/{userId}/ai
            messagingTemplate.convertAndSend(
                    "/topic/user/" + response.getUserId() + "/ai",
                    response
            );
        } catch (Exception e) {
            log.error(" Lỗi khi đọc Kafka message từ AI: {}", e.getMessage());
        }
    }
}