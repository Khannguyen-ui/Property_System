...
public class GeminiChatService {
    ...
    public String callGemini25Flash(String systemPrompt, String userMessage) {
        ...
        try {
            ...
            if ("MAX_TOKENS".equals(finishReason)) {
                log.warn("Phản hồi từ Gemini bị cắt cụt do vượt quá maxOutputTokens.");
                throw new RuntimeException("Phản hồi quá dài, hệ thống không thể xử lý hết.");
            }

            JsonNode parts = candidates.get(0).path("content").path("parts");
            if (!parts.isArray() || parts.isEmpty()) {
                throw new RuntimeException("Gemini không trả content parts.");
            }

            String text = parts.get(0).path("text").asText(null);
            if (text == null || text.isBlank()) {
                throw new RuntimeException("Gemini trả text rỗng.");
            }

            String cleanJson = extractJson(text);
            if (cleanJson == null || cleanJson.isBlank()) {
                throw new RuntimeException("Không tìm thấy JSON hợp lệ.");
            }
    ...
    public String geminiFallback(String systemPrompt, String userMessage, Exception ex) {
        log.warn("Kích hoạt fallback Gemini do lỗi hoặc quá tải: {}", ex.getMessage());

        return """
            {
              "summary": "Hệ thống AI đang quá tải, vui lòng thử lại sau ít phút.",
              "selectedPropertyIds": []
            }
            """;
    }
