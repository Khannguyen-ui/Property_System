package com.homeverse.payment.security;
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        try {
            if (Boolean.TRUE.equals(redisTemplate.hasKey("BLACKLIST:" + token))) {
                log.warn(" [Payment Service] Kẻ gian dùng Token đã bị Logout/Thu hồi. Chặn ngay!");
            ...
            if (jwtValidator.validateToken(token)) {
                ...
                try {
                    ...
                    if (Boolean.TRUE.equals(redisTemplate.hasKey(staleKey))) {
                        log.warn(" [Payment Service] Token của {} đã để quá hạn, ép Frontend đổi Token mới!", userEmail);
                        response.setStatus(HttpServletResponse.SC_UNAUTHORIZED);
                        response.setContentType("application/json;charset=UTF-8");
                ...
                if (userId != null && SecurityContextHolder.getContext().getAuthentication() == null) {
                    // Dùng List.of(...) để cấp Role thay vì Collections.emptyList() như bản cũ
                    ...
                    log.info(" [Payment Service] Xác thực thành công cho ID: {} (Email: {}, Role: {})", userId, userEmail, role);
                }
            }
        } catch (Exception e) {
            log.error("Lỗi xác thực JWT Payment: {}", e.getMessage());
        }
